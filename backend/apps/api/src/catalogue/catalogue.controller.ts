...
export class CatalogueController {
  ...
  @ApiOperation({
    ...
    description:
      'Fetch vehicle models from external API by make_id and make_year.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Fetch vehicle variants from external API by model_id and make_year.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Fetch vehicle variants with fuel_type and transmission_type from external API.',
  })
  @ApiQuery({ name: 'model_id', required: true })
  @ApiQuery({ name: 'make_year', required: true })
  @ApiResponse({
  ...
  async getVariantsFull(
    ...
    const data = await this.catalogueService.getVariantsFull(
      model_id,
      make_year,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Fetch all config filter options in a single call, formatted as {label, value} arrays keyed by field name.',
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Full config retrieved successfully',
  })
  @ApiResponse({
    status: HttpStatus.BAD_GATEWAY,
    description: 'Failed to fetch from external API',
