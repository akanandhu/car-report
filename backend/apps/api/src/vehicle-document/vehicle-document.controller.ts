...
export class VehicleDocumentController {
    ...
    try {
      ...
      if (existing) {
        ...
        return this.service.repository.create({
          ...dto,
          status: dto.status ?? VehicleDocumentStatus.DRAFT,
      ...
      console.error('VehicleDocument createOrUpdate failed', error);
  ...
  @ApiOperation({
    ...
    description:
      'Save or update form data for a specific step. Data is merged with existing draft data. Creates a DRAFT document.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Retrieve all saved form data (drafts and submitted) for a vehicle, optionally filtered by form type',
  })
  @ApiParam({ name: 'vehicleId', description: 'Vehicle ID' })
  @ApiQuery({
    name: 'type',
    required: false,
    description: 'Form type to filter by (e.g., EVALUATION, FINANCE)',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Changes all DRAFT documents for the given type to SUBMITTED status',
  })
  @ApiParam({ name: 'vehicleId', description: 'Vehicle ID' })
  @ApiParam({
    name: 'type',
    description: 'Form type (e.g., EVALUATION, FINANCE)',
  })
  ...
  async submitAllSteps(
    ...
    ResponseDto<{ message: string; submittedCount: number; totalSteps: number }>
  > {
    const result = await this.service.submitAllSteps(
      vehicleId,
      type,
      dto.submittedBy,
    );
