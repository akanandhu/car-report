...
export class SharedAuthTokenService {
  ...
  async generateTokens(userId: string) {
    ...
    const roles = userRoles.map((userRole) => userRole.role.identifier);
   ...
   */
  verifyAccessToken(token: string): TokenPayloadI {
    try {
      return this.jwtService.verify<TokenPayloadI>(token);
    } catch {
