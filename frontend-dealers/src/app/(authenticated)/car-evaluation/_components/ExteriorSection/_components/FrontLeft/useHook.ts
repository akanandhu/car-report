...
const getDefaultValues = (data: CarEvaluationFormDataI): ExteriorFormData => ({
  ...
  fullBodyRepaint: data.fullBodyRepaint ?? "",
});
...
const useFrontLeft = ({
  ...
  const missingParts = watch("missingParts") ?? "";
  const fullBodyRepaint = watch("fullBodyRepaint") ?? "";
  ...
  const updateSimpleValue = (name: "missingParts" | "fullBodyRepaint", value: string) => {
