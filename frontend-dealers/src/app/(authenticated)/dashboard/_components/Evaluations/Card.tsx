...
const Card = ({ vehicle }: CardProps) => {
  ...
  return (
    <div
      data-slot="card"
      className="bg-white text-card-foreground flex flex-col gap-6 rounded-2xl border py-6 shadow-md overflow-hidden hover:shadow-lg transition-shadow"
    >
      <div className="p-4">
        ...
        <div className="space-y-2 mb-4 text-sm mt-5">
          <div className="flex justify-between">
            <span className="text-slate-600 font-thin">Model:</span>
            <span className="text-black font-medium">{vehicle.model}</span>
          </div>
          <div className="flex justify-between">
        ...
        <div className="flex gap-2 justify-center">
          {isDraft ? (
            <Link href={`/car-evaluation?vehicleId=${vehicle.id}`} className="w-full">
              <Button
                variant="filled"
                size="sm"
                className="py-1 h-9 px-10 w-full"
              >
                Continue Evaluation
              </Button>
            </Link>
            ...
            <>
              ...
              <Button
                ...
                size="sm"
                className="h-9 px-9"
                startAdornment={<Share className="h-4 w-4" />}
              >
                Share
              </Button>
              <Button variant="outlined" className="h-9 px-3" size="xs">
